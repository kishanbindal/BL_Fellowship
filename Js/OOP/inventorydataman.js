...
class InventoryManager {
    ...
    getInventory() {
        ...
        for (var item in file.inventory) {
            ...
            let data = this.getInventoryData(items);
    ...
    editInventory() {
        let item_name = rl.question('Enter Name of Item you would like to Edit? \n');
        for (var i = 0; i < this.items.length; i++) {
            if (this.items[i]['item'].toLowerCase() == item_name.toLowerCase()) {
                while (true) {
                    ...
                    let choice = parseInt(rl.question('Enter Choice (1-3)'));
                    if (choice == 1) {
